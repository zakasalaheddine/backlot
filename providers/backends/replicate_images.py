...
# Aspect ratios nano-banana accepts natively (fallback if the model's profile
# doesn't list its own). Anything else must be produced by post-crop (the ad
# runner handles that) rather than asking the model.
NATIVE_ASPECTS = {
...
def _aspect(aspect: str, profile: dict) -> str:
    native = set((profile or {}).get("aspects") or NATIVE_ASPECTS)
    return aspect if aspect in native else "match_input_image"


def _run(prompt: str, ref_imgs, aspect: str, out_path: Path, output_format: str,
         model: str, profile: dict) -> Path:
    handles = [open(p, "rb") for p in (ref_imgs or [])]
    try:
        input_dict = {
            "prompt": prompt,
            "aspect_ratio": _aspect(aspect, profile),
            "output_format": output_format,
        }
        if handles:
            input_dict["image_input"] = handles
        return run_image(model, input_dict, out_path)
    finally:
        for h in handles:
            h.close()
...
def generate_reference(prompt: str, angle: str, aspect: str, out_path: Path,
                       ref_imgs=None, *, model: str, profile: dict = None) -> Path:
    ...
    return _run(full, ref_imgs, aspect, out_path, "png", model, profile or {})


def composite(prompt: str, negative: str, ref_imgs, aspect: str, out_path: Path,
              *, model: str, profile: dict = None) -> Path:
